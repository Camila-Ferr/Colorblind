...
# carregar imagens
# bala
bullet_img = pygame.image.load('Sprites/Elementos/coracao.png').convert_alpha()

# bomba
bomba_img = pygame.image.load('Sprites/Elementos/Moeda.png').convert_alpha()
...
class Personagens(pygame.sprite.Sprite):
    def __init__(self, char_tipo, x, y, scale, velocidade, municao , bombas):
        ...
        tipos_animacao = ['parado', 'move', 'pulo', 'morto']
    ...
    def atirar(self):
        if self.tiro_cooldown == 0 and self.municao > 0:
            ...
            tiro = Bullet(self.rect.centerx + (0.6 * self.rect.size[0] * self.direction), self.rect.centery,
                          self.direction)
